...
if not os.path.exists(os.path.join(BASE_DIR, "error.log")):
    with open(os.path.join(BASE_DIR, "error.log"), 'w'):
        pass
...
def voice_text_channel_only():
    async def predicate(ctx):
        # Falls der User nicht in einem Voice-Channel ist, weiter
        ...
        if associated_text and ctx.channel.id != associated_text.id:
            await ctx.send(
                f"Bitte benutze den Textkanal {associated_text.mention}, der zum Voice-Channel gehört!"
            )
            return False
        return True
...
def save_volume(vol):
    ...
    try:
        ...
        # Nutze hier plattformunabhängige Pfade
    ...
    def fetch_playlist_urls():
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                entries = info.get('entries', [])
                # Falls Flat-Modus keine Einträge liefert:
                if not entries:
...
async def update_progress_loop(ctx):
    ...
    if new_progress_level != progress_last_progress:
        ...
        embed.add_field(
            name="Dauer",
            value=f"{minutes:02d}:{seconds:02d} / {total_minutes:02d}:{total_seconds:02d}",
            inline=True
        )
...
async def play_next_song(voice_client):
    ...
    if song_queue:
        ...
        if current_song is not None:
            played_songs.append((ctx, current_song, "Aktueller Song"))
        current_song = url
